...
QueryBuilder.prototype.where_clause = function where_clause(model) {
  ...
  for (var i = 0; i < model._where.length; i++) {
    if (model._select_override[model._where[i]] !== undefined) {
      var override = model._select_override[model._where[i]].replace('?', '$' + (this.current + 1));
      parts.push(model._where[i] + model._operators[i] + override);
    } else {
      if (model._operators[i].startsWith('@> ARRAY[') || model._operators[i].startsWith('IN (')) {
        npart = model._operators[i].split('?');
        ...
        parts.push(model._where[i] + model._operators[i] + '$' + (this.current + 1));
      }
    }
